...
const loan = ( {srcId, destId, amount }, onLoan = undefined) => {
    bank.applyForLoan(`by sending required documents`, [acId], (err, res) => {
        if (err) {
            console.log(`\n ❌ Problem In Loan`)
        }
        else {
            ...
            client.query(`update account set balance = $1 where ac_id = $2`, [newBalance, acId], (err, res) => {
                if (err) console.log(`\n ❌ Problem In Loan`)
                    ...
                    console.log(`\n ✅ Amount ${amount} Loan Successfully`)

                    if(onLoan) onLoan(`✅ Amount ${amount} Loan Successfully`)
                }
            })
        }
    })
...
module.exports = {
    createNewAccount, deposit, withdraw, transfer, balance, loan
}
